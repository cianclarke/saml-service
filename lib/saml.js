...
router.post('/login/callback', function(req, res){
  var mw = passport.authenticate('saml', {
    failureRedirect: '/',
    failureFlash: true
  }, function(err, samlResult){
    // Some passport-saml error won't cause a redirect, but rather throw an error
    if (err){
      return res.status(401).json({ error : err.toString(), message : 'Error authenticating with SAML server' });  
    }
    // Persist our SAML assertion for this user via $fh.db
    req.user = samlResult;
    return processToken(req, function(err) {
